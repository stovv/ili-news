...
import { DoublePostsLine, DateNews } from "../../infinityBlocks";
...
const Initial = Object.values({...DateNews("news-date", 1, {isFirst: true})})[0];
    ...
    async ({store, ...props}) => {
        ...
        if ( typeof rubric === "number") {
            ...
            const postsCount = await getPostCountInRubric(rubric.id)
                ...
                .catch(reason => {
                    console.log("Something went wrong in getting posts count by news -> ", reason);
                    return 0;
                });
                    ...
                    <JournalHeader>{title}</JournalHeader>
                    {
                        postsCount === 0
                            ...
                            : <div className={containers.CommonContainer}>
                                <SideContainer sideChildren={
                                    <AdBanner vertical height={"90vh"}/>
                                }>
                                    {
