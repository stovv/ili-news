...
export async function getDraft(slug, jwt) {
  return api.get(`/drafts/?slug=${slug}`, {
    headers: {
...
export async function loadPosts(
  ...
  return api.ql(`
    ...
    rubric !== null
      ...
      : ""
  }${additionalWhere} }, limit: ${limit}, start: ${start}) {
            id,
            event{
                date
            },
...
export async function fetchPostsCount(
  ...
  const rubricQuery =
    skipRubricIds.length > 0
      ? skipRubricIds.map((id) => `rubric_nin=${id}`).join("&")
      : null;
  const categoryQuery =
    skipCategoryIds.length > 0
      ? skipCategoryIds.map((id) => `category_nin=${id}`).join("&")
      : null;
...
export async function fetchSimplePosts(
  skipPostIds = [],
  skipRubricIds = [2],
  skipCategorieIds = [],
  limit = 2
...
export async function fetchFrontPageCategories(
  ...
  const rubricQuery =
    ignoreRubrics.length > 0
      ? `rubrics_nin: [${ignoreRubrics.join(",")}]`
      : null;
  const categoryQuery =
    ignoreCategories.length > 0
      ? `id_nin: [${ignoreCategories.join(",")}]`
      : null;
  const where_data = [rubricQuery, categoryQuery].filter((q) => q !== null);
...
export function randomUnsplashImage(keyword) {
