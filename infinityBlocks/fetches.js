...
export async function fetchCategoryLine({ availableCategories: categories, existsPostIds,
                                    prevCategoryIds, categoryOffsets, dispatch }){
    let catLine = { posts: [] };
...
export async function fetchPostLine({count = 4, categoryOffsets,
    ...
    if (category !== undefined){
        let start = categoryOffsets[category] !== undefined ? categoryOffsets[category] : 0;
        await fetchCatPosts(count, category, [], start)
            .then(response => {
                if (response.data.posts != null && response.data.posts.length > 0) {
                    ...
                    dispatch(setCategoryOffset(category, start + response.data.posts.length));
                }
            })
